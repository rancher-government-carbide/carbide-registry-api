...
func imageIDFromPath(w http.ResponseWriter, r *http.Request) int32 {
	...
	if err != nil {
		httpJSONError(w, "invalid image ID", http.StatusBadRequest)
		log.Error(err)
		return -1
	}
...
func createImageHandler(db *sql.DB) http.HandlerFunc {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		err := decodeJSONObject(w, r, &newImage)
		if err != nil {
			log.Error(err)
		}
...
func updateImageHandler(db *sql.DB) http.HandlerFunc {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		err := decodeJSONObject(w, r, &updatedImage)
		if err != nil {
			log.Error(err)
			return
		}
